function results = run_DeepSTRCF_BY3(seq, res_path, bSaveImage)
setup_paths();

%weight of channels by JW
params.mean_factor = [0.90,0.90];
params.mean_yz = [1.75,    1.55];
params.resp_w=[1,    1.98,    1.49];  %响应图权重
params.layers = [29,20];
params.maxby_iters = 2;
params.pos_diff = 1;
...
params.search_area_scale = 4.5;         % The scaling of the target size to get the search area VOT中是4
...
params.temporal_regularization_factor = [15 15 15]; % The temporal regularization parameters时间正则化参数，VOT中是8

% ADMM parameters
params.max_iterations = [2 2 2];
params.init_penalty_factor = [1 1 1];
params.max_penalty_factor = [0.1, 0.1,0.1];
params.penalty_scale_step = [10, 10, 10];
